...
{ //FUNCION RECIBIR MENSAJE
    ...
    for (;;)
    {
        if (msgrcv(p->msqid, &buf, sizeof(buf.msg), 1, 0) == -1)
        {
            perror("error en msgrecieve");
            exit(8);
        }
        printf("Cornejo:  %s \n", buf.msg);
        holder = strcmp(buf.msg, "nea");
        if (holder == 0)
        {
            break;
        }
...
int main(void)
{
    ...
    pthread_create(&henvia, NULL, &enviarmsg, &paramh1);
    pthread_create(&hrecibe, NULL, &recibirmsg, &paramh2);
